...
function DemoBox({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="font-din text-[9px] tracking-[0.4em] uppercase mb-3" style={{ color: TEAL }}>
        {label}
      </p>
      <div
...
export default function PreviewScroll() {
  ...
  return (
    <main className="min-h-[300vh] bg-[#080808] px-8 py-12">
      <p className="font-din text-[10px] tracking-[0.4em] uppercase text-white mb-2">
        Scroll indikátor — varianty
      </p>
      <p className="font-din text-[9px] text-[#444] mb-10 tracking-widest uppercase">
          ...
          {(() => {
            ...
            return (
              <div className="absolute bottom-4 right-4 w-12 h-12 flex items-center justify-center">
                ...
                <span className="absolute font-din text-[8px] font-bold" style={{ color: TEAL }}>
                  {Math.round(live)}%
                </span>
              </div>
            );
