...
function Avatar({ member }: { member: { initials: string; name: string; photo?: string } }) {
  ...
  return (
    <div
      className="w-20 h-20 rounded-full flex items-center justify-center font-montserrat text-black text-2xl flex-shrink-0"
    ...
    <main className="min-h-screen pt-14" style={{ background: "#060606" }}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          {members.map((m, i) => (
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-white font-montserrat font-bold text-sm leading-snug">
                  {m.name}
                </p>
                <p className="font-montserrat text-[10px] uppercase tracking-widest mt-0.5 mb-2" style={{ color: "#C89A2A" }}>
                  {m.role}
                </p>
                {m.bio && (
                  <p className="font-montserrat text-[11px] leading-relaxed" style={{ color: "#888" }}>
                    {m.bio}
                  </p>
                )}
              </div>
        ...
        <div className="mt-12 pt-8 border-t" style={{ borderColor: "#1a1a1a" }}>
          <Link
            href="/"
            className="font-montserrat text-[11px] tracking-[0.2em] uppercase hover:text-white transition-colors duration-200"
            style={{ color: "#555" }}
          >
